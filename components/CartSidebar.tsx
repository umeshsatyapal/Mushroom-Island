...
import { useCart } from '../context/CartContext'; // Correct path to context
...
const CartSidebar: React.FC<CartSidebarProps> = ({ onNavigate }) => {
    ...
    // If "isCartOpen" is false, we don't show anything
    ...
    return (
        <div className="fixed inset-0 z-[100] flex justify-end">
            ...
            {/* 2. The Sliding Panel (Emerald Green) */}
            <div className="relative w-full max-w-md bg-[#0F281E] h-full shadow-2xl flex flex-col text-white transform transition-transform duration-300">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    {items.length === 0 ? (
                        <p className="text-gray-400 text-center mt-10 italic">Your cart is currently empty.</p>
                    ) : (
                        items.map((item) => (
                            <div key={item.id} className="flex gap-4 items-start border-b border-white/5 pb-4 last:border-0">
                                {/* Product Image */}
